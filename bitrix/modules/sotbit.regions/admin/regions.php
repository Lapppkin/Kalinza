...
$filterFields = array('find_i', 'find_c','find_n','find_s','find_site');
$filterValues = array();
$filterTitles = array('ID', 'CODE','NAME','SORT','SITE_ID');
...
if($lAdmin->EditAction())
{
	foreach($FIELDS as $ID=>$arFields)
	{
		...
		unset($arFields['PRICE_CODE']);
		unset($arFields['STORE']);
		...
		while ($field = $rs->fetch())
		{
			...
			{
				if(is_array($arFields[$field['CODE']]))
				{
					$arFields[$field['CODE']] = serialize($arFields[$field['CODE']]);
				}
...
foreach($data as $id => $arRes)
{
	...
	$arActions[] = array(
		...
		'ACTION' => 'if(confirm(\''.Loc::getMessage(\SotbitRegions::moduleId.'_DELETE_CONFIRM').'\')) '.
			$lAdmin->ActionRedirect('sotbit_regions_edit.php?action=delete&ID='.$arRes['ID'].'&lang='.LANGUAGE_ID.'&'
				.bitrix_sessid_get())
	);
...
$menu[] = array(
	...
	'LINK'	=> 'sotbit_regions_edit.php?lang='.LANGUAGE_ID,
	'ICON'	=> 'btn_new'
);

$lAdmin->AddAdminContextMenu($menu,false);
