...
if (isset($_REQUEST['ID']) && $_REQUEST['ID'] > 0 && isset($allRegions) && is_array($allRegions)) {
    foreach ($allRegions as $key => $region) {
        if($_REQUEST['DEFAULT_DOMAIN'] == 'Y' && $region['DEFAULT_DOMAIN'] == 'Y' && $_REQUEST['ID'] !=  $region['ID'])
            $arrKeyClean[] = $key;
...
if(empty($row)){
    ...
    }else if(!isset($_REQUEST["DEFAULT_DOMAIN"]) && !$_REQUEST["DEFAULT_DOMAIN"] && !empty($allRegions)){
...
if ($_REQUEST['point_map_google_sotbit_regions_code2__n0_lat']) {
    ...
    $row["MAP_GOOGLE"] = ['VALUE' => $mapG, 'API_KEY' => $_REQUEST['GOOGLE_API_KEY']];
}
...
if ((strlen($save) > 0 || strlen($apply) > 0) && $REQUEST_METHOD == 'POST'
    ...
    if((!isset($_REQUEST['DEFAULT_DOMAIN']) || $_REQUEST['DEFAULT_DOMAIN'] == 'N') && empty($arrKeyClean) && !empty($allRegions)){
    ...
    if ($ID > 0 && $data) {
        foreach (array_keys($data) as $code) {
            if (is_array($data[$code])) {
                $data[$code] = array_values($data[$code]);
    ...
    <tr id="SITE_ID">
        ...
        <td width="60%">
            ...
            foreach ($sites as $lid => $site) {
                ?>
                <input
                type="checkbox" <?= ($row['SITE_ID']
                && is_array($row['SITE_ID'])
                && in_array($lid, $row['SITE_ID'])) ? 'checked' : '' ?>
                name="SITE_ID[]"
                value="<?= $lid ?>"><?= $site ?><br>
                <?php
            }
...
} else if(Loader::includeModule('sale')){
