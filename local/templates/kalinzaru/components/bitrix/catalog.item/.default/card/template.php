...
<div class="product-item">
    <a class="product-item-image-wrapper" href="/catalog<?= $item['DETAIL_PAGE_URL'] ?>" title="<?= $imgTitle ?>"
       data-entity="image-wrapper">
        ...
        <span class="product-item-image-original" id="<?= $itemIds['PICT'] ?>"
              style="background-image: url('<?= $resizeImg['src']; ?>'); <?= ($showSlider ? 'display: none;' : '') ?>">
		</span>
        <?
        if ($item['SECOND_PICT']) {
            $img     = $item['PREVIEW_PICTURE_SECOND'] ?? $item['PREVIEW_PICTURE'] ?? null;
            $bgImage = 'data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=';

            if ($img) {
                $resizeImg = \CFile::ResizeImageGet($img['ID'], ['width' => 200, 'height' => 9999], BX_RESIZE_IMAGE_PROPORTIONAL_ALT);
                $bgImage   = $resizeImg['src'];
            }
            ?>
